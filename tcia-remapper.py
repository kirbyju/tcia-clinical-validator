        ...
        if import_file:
            try:
                ...
                if not import_df.empty:
                    ...
                    # Map Dataset
                    proposal_data_cleaned = {}
                    for k, v in proposal_data.items():
                        if pd.isna(v) or str(v).lower() == 'nan': proposal_data_cleaned[k] = ""
                        elif isinstance(v, str) and v.startswith('[') and v.endswith(']'):
                            try: proposal_data_cleaned[k] = ast.literal_eval(v)
                            except: proposal_data_cleaned[k] = v
                        else: proposal_data_cleaned[k] = v

                    ds_data = {
                        'dataset_long_name': proposal_data_cleaned.get('Title', ''),
                        'dataset_short_name': proposal_data_cleaned.get('Nickname', ''),
                        'dataset_abstract': proposal_data_cleaned.get('Abstract', ''),
                        'dataset_description': '',
                        'adult_or_childhood_study': proposal_data_cleaned.get('adult_or_childhood_study', []),
                        'acknowledgements': proposal_data_cleaned.get('acknowledgements') or proposal_data_cleaned.get('acknowledgments') or '',
                        'funding_agency': proposal_data_cleaned.get('funding_agency', ''),
                        'funding_source_program_name': proposal_data_cleaned.get('funding_source_program_name', ''),
                        'grant_id': proposal_data_cleaned.get('grant_id', ''),
                        'why_tcia': proposal_data_cleaned.get('why_tcia', [])
                    }
                    ...
                    if entity_key in ["Investigator", "Related_Work"]:
                        for idx, item in enumerate(entity_data):
                            ...
                            for key, value in item.items():
                                display_val = ", ".join(map(str, value)) if isinstance(value, list) else (str(value) if not pd.isna(value) and str(value).lower() != "nan" else "")
...
# ============================================================================
    ...
    st.markdown("Follow the [CICADAS checklist](https://cancerimagingarchive.net/cicadas) to ensure your dataset is comprehensive and optimally discoverable.")
    with st.form("cicadas_form"):
        st.write("### Abstract")
        ...
        if st.form_submit_button("Save & Next"):
            ...
            m = ""
            if c_m_subjects: m += f"### Subject Inclusion and Exclusion Criteria\n{c_m_subjects}\n\n"
            if c_m_acquisition: m += f"### Data Acquisition\n{c_m_acquisition}\n\n"
            if c_m_analysis: m += f"### Data Analysis\n{c_m_analysis}\n\n"
            if m: desc.append(f"## Methods\n{m}")
            ...
            st.toast("✅ CICADAS saved!"); st.session_state.phase = 2; st.rerun()
...
# ============================================================================
    ...
    uploaded_file = st.file_uploader("Upload your source data file", type=['csv', 'tsv', 'xlsx', 'xls'])
    if uploaded_file is not None:
        try:
            if uploaded_file.name.endswith('.csv'): df = pd.read_csv(uploaded_file)
                ...
                df = pd.read_excel(uploaded_file, sheet_name=st.selectbox("Select sheet", xl.sheet_names) if len(xl.sheet_names) > 1 else xl.sheet_names[0])
            if df is not None:
            ...
            for col in df.columns:
                ...
                with c1: st.write(col)
                with c2:
                    sel = st.selectbox("Target", options=["(Skip)"] + list(all_props.keys()), key=f"m_{col}")
                    if sel != "(Skip)": mapping_data.append((sel, col))
            if st.button("Confirm Mapping"):
                st.session_state.column_mapping = {t: s for t, s in mapping_data}; st.session_state.structure_approved = True
                st.rerun()
            if st.session_state.structure_approved:
                st.subheader("Value Standardization")
                split_data = split_data_by_schema(st.session_state.uploaded_data, st.session_state.column_mapping, schema)
                for e, edf in split_data.items():
                    with st.expander(f"📊 {e}"):
                        ...
                        if rep:
                            ...
                            if corr and st.button(f"Apply Corrections to {e}"):
